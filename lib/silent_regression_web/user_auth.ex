defmodule SilentRegressionWeb.UserAuth do
  ...
  @remember_me_cookie "_silent_regression_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  """
  def log_in_user(conn, user, params \\ %{}, auth_method \\ "password") do
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      SilentRegressionWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def signed_in_path(user) do
    case Workspaces.default_workspace_scope(Scope.for_user(user)) do
      {:ok, %Scope{workspace: workspace}} -> "/app/#{workspace.slug}"
      {:error, :not_found} -> ~p"/"
    end
  end
