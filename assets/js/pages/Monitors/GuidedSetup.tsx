...
type Proof = { id: string; fingerprint: string; caseKey: string; name: string; inputJson: string; context: string; expected: string; output: string; proposedShared: string; proposedCase: string; shared: string; specific: string; reason: string }
export type GuidedSetupProps = {
  auth: SharedPageProps["auth"]; step: Step;
  draft: { id: string; revision: number; rawJson: string; reviewedIds: string[] };
...
export function GuidedSetupView({ auth, step, draft, journey, models, credentials, errors = {}, flash = {} }: GuidedSetupProps & { errors?: Record<string, string>; flash?: SharedPageProps["flash"] }) {
  const form = useForm({ raw: JSON.parse(draft.rawJson) as RoutingRaw, revision: draft.revision })
  ...
  function update<K extends keyof RoutingRaw>(key: K, value: RoutingRaw[K]) { form.setData("raw", { ...raw, [key]: value }); setConfirmed([]) }
  ...
  return <>
    <Head title={`${titles[position]} · saved routing setup`} />
    <ProductShell currentSection="monitors" releaseStage="Private alpha" availableWorkspaces={auth.workspaces} userEmail={auth.user?.email || "Invited user"} workspace={auth.workspace} membershipRole={auth.membership?.role || "member"}>
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex flex-wrap items-center gap-3"><Badge variant="outline">Real monitor draft</Badge><p className="text-sm text-muted-foreground">Saved in your workspace · no calls while authoring</p></div>
        ...
        {journey.blockers.length > 0 && <aside className="rounded-xl border bg-muted/30 p-4 text-sm leading-6"><p className="font-medium">{dirty ? "Last saved check — save to validate your edits" : "Before you continue"}</p>{journey.blockers.map(message => <p key={message}>{message}</p>)}</aside>}
        ...
        {step === "examples" && <Panel title="Labels and known answers" description="List the outputs allowed in general, then the one correct answer for each input. These are your expectations—not answers inferred from a model.">
          <Field id="routing-labels" label="Allowed labels — one per line"><Textarea id="routing-labels" value={raw.labelsText} onChange={event => update("labelsText", event.target.value)} placeholder={"approved\nrejected"} /><p className="text-xs text-muted-foreground">2–8 labels. Whole-output match after Unicode normalization, case folding, and punctuation/whitespace normalization. No paraphrase matching.</p></Field>
          {raw.cases.map((row, index) => <fieldset key={row.key} className="space-y-4 rounded-xl border p-4"><legend className="px-1 font-medium">Example {index + 1}</legend>
            ...
            <ChoiceSelect id={`case-expected-${index}`} label={`Correct label for example ${index + 1}`} value={row.expected} options={[...new Set(labels)]} onChange={expected => updateCase(index, { expected })} />
          ...
          <p className="text-sm leading-6 text-muted-foreground">Use representative inputs where you know the correct answer. A correct rejection is a passing case. Unknown semantic correctness is outside this routing recipe.</p>
        </Panel>}
        ...
        {step === "checks" && <Panel title="Review proposed good and bad outputs" description="These synthetic outputs test your checks locally using the production evaluators. They are not provider results. Read each proposal and confirm only judgments you agree with.">
          <p className="text-sm leading-6">Shared check: the whole answer is an allowed label. Case check: it is the correct label for this input. A wrong allowed label must pass the shared check and fail the case check.</p>
          {journey.proof.map((row, index) => <article key={row.id} className="space-y-3 rounded-xl border p-4" aria-label={`Proof ${index + 1}`}>
            ...
            <dl className="grid gap-3 sm:grid-cols-2"><div><dt className="text-xs text-muted-foreground">Correct label</dt><dd className="break-all font-mono">{row.expected}</dd></div><div><dt className="text-xs text-muted-foreground">Proposed output</dt><dd className="break-all font-mono">{row.output}</dd></div></dl>
